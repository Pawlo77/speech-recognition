...
\begin{frame}{Phase 3: Failure Highlight}
    ...
    \slidecomment{Collapse-rate analysis confirms reliability differences: ConvNeXt has the fewest severe failures, MLP-Mixer the most. Results with $\text{F1}<0.5$, worse than random.}
\end{frame}
...
\begin{frame}{Phase 4: Silence False Trigger Rate}
    ...
    \slidecomment{Two-stage detector shows a critical failure mode with elevated silence false triggers, which is unacceptable for always-on KWS. Sampling control and flat multiclass have the best silence robustness.}
\end{frame}
