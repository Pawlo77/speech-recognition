...
\begin{frame}{Phase-Wise Architectures and Approaches}
    \small
    \begin{itemize}
        \item \textbf{Phase 1 (Feature selection):} comparison on \textbf{ConvNeXt + xLSTM} across mel/mfcc/pcen/specaugment pipelines.
        \item \textbf{Phase 2 (Global tuning):} same proxy families with \textbf{scheduler + weight decay} sweep (cosine warmup vs reduce-on-plateau).
        \item \textbf{Phase 3 (Architecture comparison):} full comparison of \textbf{ConvNeXt, AST, MLP-Mixer} (SSAMBA/xLSTM kept as planned, not executed).
        \item \textbf{Phase 4 (Open-set handling):} top backbones retrained with \textbf{flat multiclass, sampling control, loss reweighting, two-stage}.
    \end{itemize}
\end{frame}

\begin{frame}{Models and Methods}
    \small
    \textbf{Feature strategies (Phase 1)}
    \begin{itemize}
...
\begin{frame}{Models and Methods}
    ...
    \end{itemize}
    \vspace{0.2em}
    \textbf{Planned but unused in executed Phase 3}
    \begin{itemize}
        \item \textbf{xLSTM}: recurrent sequence model, used in early phases and kept in repository scope, dropped due to poor performance.
        \item \textbf{SSAMBA/Mamba}: selective state-space sequence model, implemented but not part of executed Phase 3 runs due to computational constraints.
    \end{itemize}
    \vspace{0.2em}
\end{frame}
...
\begin{frame}{Phase 2: Convergence Speed}
    ...
    \slidecomment{A lot slower training for xLSTM. Stable convergence for ConvNeXt across all settings, with ReduceLROnPlateau slightly faster than cosine warmup.}
\end{frame}
...
\begin{frame}{Phase 3: AST Ablation (Head / Embedding / Dropout)}
    \centering
    \includegraphics[height=0.68\textheight,keepaspectratio]{phase_3_ast_ablation.png}
    \slidecomment{Macro F1 across AST head and embedding pairs, split by dropout; differences are moderate and best configs remain in a similar band.}
\end{frame}
...
\begin{frame}{Training Efficiency (ConvNeXt vs AST)}
    ...
    \slidecomment{AST often converges in fewer epochs, but ConvNeXt has a lot faster total training time and better final F1.}
\end{frame}

\begin{frame}{Phase 3: Training Loss vs Epoch}
    \centering
    \includegraphics[height=0.68\textheight,keepaspectratio]{phase_3_training_loss_vs_epoch.png}
    \slidecomment{ConvNeXt shows the strongest and most consistent training loss reduction across epochs, AST shows slower and more variable convergence, while MLP-Mixer has the weakest training dynamics.}
\end{frame}
