...
\textbf{$3$} & Inductive Bias Comparison \newline (AST: $8$, ConvNeXt: $2$, SSAMBA: $8$, MLP-Mixer: $4$; planned xLSTM excluded) & $22$ & $3$ & $66$ \\
...
\textbf{Total} & \textbf{Cumulative Trial Executions} & \textbf{$55$} & & \textbf{$165$} \\ \bottomrule
\end{tabular}
\end{table}
...
\begin{table*}[htbp]
\centering
\caption{Model-specific hyperparameter sweeps (Phase $3$), including architecture type and parameter count from the repo-specific adapters used in this work (default non-pretrained configuration, $12$ output classes). xLSTM is retained as planned scope but excluded from executed runs.}
...
\textbf{SSAMBA} & State-space sequence model & $23{,}963{,}148$ & Pooling \{mean, max\}; CLS \{yes, no\}; Stride $\{10\text{ms}, 5\text{ms}\}$; fixed size: $d_{\text{model}}=768$, $d_{\text{state}}=64$, expand $=2$, layers $=6$ \cite{gu2023mamba,shams2024ssamba} \\
