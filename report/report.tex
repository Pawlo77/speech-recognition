...
Before merging labels, we persist a metadata artifact \texttt{train/split\_lists/unknown\_origin\_labels.csv} that maps each merged unknown file to its original source label, ensuring no provenance is lost. For the extended dataset configuration, the unknown pool is built from (i) original labels outside the $10$ target commands and outside silence/background-noise, and (ii) synthetic \texttt{\_\_unknown\_\_} mixtures generated from those same non-target source labels. Candidate source clips are constrained to come from different classes within one synthetic sample.
